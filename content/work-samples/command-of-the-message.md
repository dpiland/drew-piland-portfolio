...
Same discipline as everywhere else in this doc: where a response leans on a number, it's one of the proof points above, and where it doesn't, that's said plainly rather than papered over.

| Objection (buyer's words) | Reframe | Proof point used | When this response doesn't apply |
|---|---|---|---|
| "We're already consolidating onto [Competitor]'s platform. Why add another layer?" | Consolidation and coordination aren't the same bet. Ask what governs the workflows that haven't migrated yet, mid-project. Unify governs the estate as it exists today, including the parts still mid-migration, so nothing goes ungoverned in the gap. | The disqualifying test, section 7 | If the estate is genuinely small and homogeneous enough for an in-quarter, single-vendor migration, say so. That buyer is better served by consolidation; don't force the frame. |
| "This is just more tooling on tooling. Why not keep coordinating this ourselves?" | That's the current state today, and it's already named as the problem: a release calendar, a spreadsheet, and a person who knows who to call. It breaks under headcount turnover and again under scale. The build-it-yourself version is the thing already failing, not an alternative to it. | Current state (section 2) and negative consequence row 3 (tribal knowledge) | If the org is small enough that one person tracking context genuinely holds, don't manufacture urgency. Test with the discovery questions in section 8 rather than assert the pain. |
| "We don't have an AI agent problem. This feels ahead of where we are." | Agreed, don't oversell it: there's no published incident data yet, and that's stated as UNPROVEN in this doc on purpose. The honest case is timing, not fear. The same runtime enforcement already governing human changes covers an agent's actions at no extra setup; retrofitting that after an agent-caused incident is the expensive path. | Explicitly none claimed; this is the one place in the doc to say "no proof yet" out loud rather than reach for a stat | If the buyer has no plan to let agents act inside pipelines in the foreseeable future, this objection is a fair reason to deprioritize the thread, not a fear tactic to push through. |
| "How do we know the Forrester numbers apply to us?" | They don't, not directly: it's a commissioned, composite-organization study, a sizing signal for the category, not a per-customer guarantee. The account-specific number comes from the discovery questions in section 8 (audit hours, incident cost), not from the TEI report. | Forrester TEI caveat, section 9 | Don't cite the TEI figures as a forecast for this account. If the buyer pushes for a hard number, that's a discovery gap to close, not an objection to argue past. |
| "What's the migration cost to get value here?" | There isn't one, that's the mechanism, not a talking point: Unify coordinates the tools already in place rather than asking teams to re-platform onto a new runner first. Governance starts on the estate as it exists on day one. | Required capability row 1 (polymorphic workflows, section 6) | If the prospect's actual pain is tool sprawl they want to *reduce*, not coordinate, that's a genuine fit question. Some buyers want fewer tools, not a coordination layer across the ones they have; don't argue them out of that goal. |
| "Why trust CloudBees's governance layer over a bigger, more established vendor?" | Fair question, and the honest answer isn't brand size, it's adoption at the level being sold to: 13 referenceable customers against a goal of 10, deal size moving from $100K to $265K, a 60% win rate once deals reach qualified. That's evidence of committee-level buy-in, not a claim about market share. | May 2025 launch results, section 9 | Don't extend this into a claim about overall market position CloudBees hasn't published. The proof point is real and specific; keep it scoped to what it actually shows. |
